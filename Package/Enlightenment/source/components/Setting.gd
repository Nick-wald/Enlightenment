...
func _GUI_init():
	...
	for package in SettingCopy.keys():
		for tab in (SettingCopy[package]["config"] as Dictionary).keys():
...
func _on_cancel_pressed() -> void:
	(Global.USENODE("GUIManager") as GUIManager).changeShowmode(
		Global.getSetting("Video", "Showmode")[0],
		Rect2i(
			0,
			0,
			int((Global.getSetting("Video", "Size")[0] as String).get_slice("x", 0)),
			int((Global.getSetting("Video", "Size")[0] as String).get_slice("x", 1))
		)
		)
	for item in (Global.getSetting("Audio") as Dictionary).keys():
		(Global.USENODE("AudioManager") as AudioManager).set_vol(item, Global.getSetting("Audio")[item])
	queue_free()
