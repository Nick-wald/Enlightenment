...
## 显示设置
# 窗口模式
var showmode:String = Global.getSetting("Video", "Showmode")[0]
# 窗口大小
...
func _ready():
	...
	changeShowmode(Global.getSetting("Video", "Showmode")[0], Rect2i(0, 0, int((Global.getSetting("Video", "Size")[0] as String).get_slice("x", 0)), int((Global.getSetting("Video", "Size")[0] as String).get_slice("x", 1))))
...
func changeShowmode(mode:String = "null", size:Rect2i = Rect2i(Vector2i(0, 0), windowSize), title:String = "Enlightenment"):
	showmode = Global.getSetting("Video", "Showmode")[0]
	if mode.is_empty():
		mode = showmode
	match mode:
		"fullscreen":
